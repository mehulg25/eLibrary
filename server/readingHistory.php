...
$getUser = mysqli_query($conn, "SELECT `role` FROM `users` WHERE `id` = '$userId'");
$get_user = mysqli_fetch_all($getUser, MYSQLI_ASSOC);

if (strcmp($get_user[0]['role'], 'ADMIN')==0) {
    $sqlQuery = "SELECT * FROM users_bookdata JOIN books ON users_bookdata.book_id=books.id WHERE users_bookdata.user_id = ".strval($_GET['userId'])." AND users_bookdata.action_type = 'READ' ORDER BY users_bookdata.book_read_timestamp DESC";
